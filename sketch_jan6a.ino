...
float Kp = 155.0;  // Пропорційний (більший імпульс при помилці 3-4°C)
float Ki = 0.1;  // Інтегральний (швидше накопичення)
float Kd = 260.0;   // Диференціальний (гасить overshoot)
...
// --- WATCHDOG ЗАХИСТУ БЖ ---
unsigned long lowTempStartTime = 0;
bool lowTempTimerActive = false;
...
void loop() {
    ...
    if (millis() - lastSensorRead > 2000) {
        ...
        // --- WATCHDOG: Перевірка захисту БЖ ---
        if (pidEnabled) {
            // Перевіряємо, чи температура нижча за цільову на 1+ градус
            // (temp > 0 страхує від хибних спрацювань, якщо датчик ще не ініціалізувався)
            if (temp > 0.0 && (targetTemperature - temp) >= 1.0) { 
                if (!lowTempTimerActive) {
                    lowTempTimerActive = true;
                    lowTempStartTime = millis();
                    LOG("[WATCHDOG] Temp drops 1+ degree. Timer 5 min started.");
                } else if (millis() - lowTempStartTime >= 300000) { // 300 000 мс = 5 хвилин
                    LOG("[WATCHDOG] Timeout! PSU might be in protection. RESTARTING ESP...");
                    delay(500); // Даємо час відправити лог
                    ESP.restart();
                }
            } else {
                // Температура в нормі, скидаємо таймер
                if (lowTempTimerActive) {
                    lowTempTimerActive = false;
                    LOG("[WATCHDOG] Temp recovered. Timer reset.");
                }
            }
        }
    }
